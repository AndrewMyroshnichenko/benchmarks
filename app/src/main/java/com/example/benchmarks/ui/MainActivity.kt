...
class MainActivity : AppCompatActivity(), OnTabSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bind?.mainViewPager?.adapter = BenchmarkTypesAdapter(
            supportFragmentManager, lifecycle, nameOfTabs
        )
        tabLayoutMediator = TabLayoutMediator(
            bind!!.mainTabLayout, bind!!.mainViewPager
        ) { tab, position -> tab.text = nameOfTabs[position] }
